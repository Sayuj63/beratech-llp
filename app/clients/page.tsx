    ...
    <>
      ...
      <section className="border-b border-ink/10 bg-bera-cream">
        <div className="mx-auto max-w-[1400px] px-5 py-20 lg:py-24">
          <p className="reveal font-mono text-[11px] tracking-[0.24em] uppercase text-bera-orange mb-4">
            ◆ Our clients
          </p>
          <h2 className="reveal font-display text-[clamp(2rem,4.5vw,3.5rem)] leading-[0.95] tracking-tightest text-ink max-w-3xl">
          ...
          <div className="mt-14 grid gap-px bg-ink/10 lg:grid-cols-2">
            {CLIENTS.map((c) => (
              <article
                key={c.n}
                className="reveal bg-bera-cream p-8 lg:p-10 flex flex-col gap-6 min-h-[260px]"
              >
                <div className="flex items-start justify-between gap-6">
                  <div>
                    ...
                    <h3 className="mt-4 font-display text-[clamp(1.8rem,3vw,2.6rem)] leading-none tracking-tightest text-ink">
                      {c.name}
                    </h3>
                  ...
                  {c.logo && (
                    <div className="relative w-16 h-16 shrink-0 bg-bera-cream/0">
                      <Image src={c.logo} alt={c.name} fill className="object-contain" sizes="64px" />
                    </div>
                  )}
                </div>
                <p className="text-[15px] leading-relaxed text-ink/75 mt-auto">{c.note}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1400px] px-5 py-20 lg:py-28">
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-12 lg:col-span-5 reveal">
              ...
              <h2 className="mt-4 font-display text-[clamp(2rem,4.5vw,3.5rem)] leading-[0.95] tracking-tightest">
                The standards that <span className="italic text-bera-amber">earn the work.</span>
              </h2>
              <p className="mt-6 text-bera-cream/80 text-[15px] leading-relaxed max-w-md">
                Drawn directly from our mission — these are the commitments our clients depend on,
                project after project.
              </p>
            </div>

            <div className="col-span-12 lg:col-span-7 grid sm:grid-cols-2 gap-px bg-bera-cream/10">
              {[
                {
                  t: 'Safety standards',
                ...
                <div key={m.t} className="bg-ink p-7 lg:p-8 reveal">
                  <p className="font-display text-xl tracking-tight">{m.t}</p>
                  <p className="mt-3 text-[14px] leading-relaxed text-bera-cream/70">{m.d}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mx-auto max-w-[1400px] px-5 py-20 lg:py-24">
          <div className="grid gap-10 lg:grid-cols-12 items-start">
            <div className="lg:col-span-5 reveal">
              ...
              <h2 className="mt-4 font-display text-[clamp(2rem,3.5vw,3rem)] leading-[1] tracking-tightest text-ink">
                Documentation on request.
              </h2>
            </div>
            <div className="lg:col-span-7 reveal text-ink/75 text-[16px] leading-relaxed max-w-2xl">
              <p>
              ...
              <Link
                href="/contact"
                className="mt-8 inline-flex items-baseline gap-3 text-[14px] font-medium text-ink group"
              >
